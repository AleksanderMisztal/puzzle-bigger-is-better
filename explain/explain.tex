...
\begin{description}
    \item[$\com (\ck, \vec{v})$:] To commit to a vector $\vec{v} = (v_0, \ldots, v_{dim - 1})$ of length $dim$. We define polynomial $a(X) = v_0 X+ v_1 X^2 + \ldots + v_{dim - 1} X^{dim}$. The committer computes and outputs the KZG commitment to $a(X)$, i.e.~$a(\tau)$.
    %
